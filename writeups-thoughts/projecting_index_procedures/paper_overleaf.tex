...
% --- Title ---
\title{\textbf{Forecasting TAVI and SAVR Procedure Volumes:\\A Demographic-Anchored Adoption Model}}
...
\begin{abstract}
\noindent
Accurate forecasting of transcatheter aortic valve implantation (TAVI) and surgical aortic valve replacement (SAVR) procedure volumes is essential for healthcare capacity planning and for estimating downstream valve-in-valve (ViV) reintervention demand. We present a six-step, demographically anchored forecasting framework that integrates national registry data, United Nations population projections, and a constrained sigmoid adoption model. The framework decomposes the forecast into a Total Addressable Market (TAM)---driven by age-stratified risk rates and population ageing---and a TAVI adoption share governed by logistic diffusion dynamics. Applied to South Korea and Singapore (2012--2024 registry data, projections to 2050), the model reveals three key findings: (1)~total aortic valve procedure demand will grow 2.7--3.4-fold by 2050, driven primarily by the expansion of the $\geq$80 population; (2)~SAVR volumes will grow in absolute terms despite declining market share, challenging assumptions that TAVI growth will reduce surgical demand; and (3)~the two countries exhibit markedly different adoption trajectories ($k = 0.502$ vs.\ $k = 0.096$), reflecting distinct healthcare system characteristics. The resulting projections serve as upstream inputs to a patient-level Monte Carlo simulation of ViV TAVI demand.
\end{abstract}
...
\subsection{Aortic Stenosis: The Clinical Problem}

Aortic stenosis (AS) is the most common acquired valvular heart disease in the developed world, affecting approximately 2--7\% of adults aged over 65 years and rising sharply to 12.4\% in those aged over 75 years. The disease results from progressive calcification and fibrosis of the aortic valve leaflets, leading to obstruction of left ventricular outflow. Without intervention, symptomatic severe AS carries a dismal prognosis: 2-year mortality rates exceed 50\% once symptoms of heart failure, syncope, or angina develop. For decades, surgical aortic valve replacement (SAVR)---performed via median sternotomy under cardiopulmonary bypass---was the sole definitive treatment for severe AS, offering substantial symptomatic and survival benefits in operable patients.

\subsection{The TAVI Revolution}

The introduction of transcatheter aortic valve implantation (TAVI) in 2002 by Cribier and colleagues fundamentally altered the therapeutic landscape. TAVI enables bioprosthetic valve deployment via a percutaneous or transapical approach without the need for open-heart surgery or cardiopulmonary bypass. The landmark PARTNER and CoreValve trials established TAVI as non-inferior---and in some populations, superior---to SAVR across a progressive broadening of risk categories:

\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item \textbf{High-risk patients} (PARTNER~1, 2010): TAVI demonstrated non-inferiority to SAVR with respect to 1-year mortality in high-risk surgical candidates, and superiority to medical therapy in inoperable patients.
    \item \textbf{Intermediate-risk patients} (PARTNER~2, 2016; SURTAVI, 2017): TAVI was shown to be non-inferior to SAVR in intermediate-risk populations, with lower rates of acute kidney injury, atrial fibrillation, and shorter hospitalisation.
    \item \textbf{Low-risk patients} (PARTNER~3, 2019; Evolut Low Risk, 2019): TAVI demonstrated non-inferiority to SAVR in low-risk patients, effectively expanding the eligible population to the vast majority of AS patients.
\end{itemize}
...
\subsection{The Asian Context: A Confluence of Demographic Forces}
...
\subsection{Limitations of Existing Forecasting Approaches}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
    \item Anchoring projections in \textbf{demographic structure}---using United Nations population projections to account for the age-specific drivers of demand.
    \item \textbf{Normalising for pandemic disruption}---applying a volume-preserved redistribution that corrects the shape of pandemic-era data while conserving observed clinical volume.
    \item Modelling \textbf{age-specific risk rates} with a saturation function that prevents biologically implausible growth.
    \item \textbf{Separating technology adoption from demographic growth}---decomposing the forecast into a demographically driven Total Addressable Market (TAM) and a sigmoid adoption share.
    \item Providing a \textbf{country-agnostic framework} that can be parameterised for different healthcare systems.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{National Registry Procedure Counts.} Annual TAVI and SAVR procedure counts stratified by five-year age band (50--54, 55--59, 60--64, 65--69, 70--74, 75--79, $\geq$80) and sex. For South Korea, raw registry data from the national Health Insurance Review and Assessment Service (HIRA) database (2012--2024) are used. For Singapore, procedure counts from the national cardiac registry (2012--2024) are used.

    \item \textbf{United Nations World Population Prospects.} Sex-specific population projections by five-year age group for the country of interest, obtained from the UN Department of Economic and Social Affairs (2024 revision). Male and female projections are summed to obtain total population by age band and year from 2012 to 2050.

    \item \textbf{Configurable Model Parameters.} The maximum TAVI adoption share ($L_{\max}$, default: 0.50), the sigmoid fitting window start year (default: 2016), and the COVID-19 redistribution window (2020--2023) are specified as model inputs.
\end{enumerate}

\subsection{Age-Band Harmonisation}

UN population projections are reported in standard five-year bands (e.g., 80--84, 85--89, \ldots, 100+). These are mapped to the registry age-band schema by summing all UN bands with a lower bound $\geq$80---including the open-ended 100+ category---into a single $\geq$80 band. For bands 50--54 through 75--79, a direct one-to-one mapping is applied.

\subsection{Step 1: Data Overview}
...
This combined series, together with the population projections for the $\geq$80 and 50--79 cohorts, provides the foundational inputs for trend analysis.

\subsection{Step 2: Pandemic-Period Volume-Preserved Redistribution}
...
We apply a \textbf{volume-preserved redistribution} that satisfies two constraints simultaneously:

\begin{enumerate}[leftmargin=*, itemsep=2pt]
    \item \textbf{Shape consistency}---the normalised series follows the pre-pandemic secular trend.
    \item \textbf{Volume conservation}---total observed procedures within the window are exactly preserved.
\end{enumerate}

A first-order polynomial (linear trend) is fitted to the pre-COVID data ($t \leq 2019$):
...
\begin{equation}
    \kappa = \frac{\displaystyle\sum_{t \in W} O(t)}{\displaystyle\sum_{t \in W} T(t)}
\end{equation}

\noindent where $W = \{2020, 2021, 2022, 2023\}$ is the pandemic window and $O(t)$ are the observed counts. The normalised values for the pandemic window are then:
...
\subsection{Step 3: Age-Stratified Risk Rate Projection}

The \textbf{procedure risk rate} $R_i(t)$ for age band $i$ in year $t$ is defined as the number of combined TAVI + SAVR procedures per unit population:

\begin{equation}
    R_i(t) = \frac{N_{\mathrm{TAVI},i}(t) + N_{\mathrm{SAVR},i}(t)}{P_i(t)}
\end{equation}

\noindent where $P_i(t)$ is the population of age band $i$ in year $t$.

Raw risk rates are first normalised using the redistribution procedure (Section~2.4). The normalised rates are then projected forward using a \textbf{logarithmic saturation model}:

\begin{equation}
    \hat{R}_i(t) = a_i + b_i \cdot \ln(t - t_{\mathrm{offset}})
\end{equation}

\noindent where $t_{\mathrm{offset}} = 2010$ and parameters $(a_i, b_i)$ are estimated by nonlinear least-squares fitting (Levenberg--Marquardt algorithm) to the historical normalised rates for each age band~$i$.

\paragraph{Rationale.}
The logarithmic functional form captures the empirical observation that treatment rates grow rapidly during the early adoption phase but decelerate as the treated fraction of the diseased population approaches the true prevalence ceiling. Unlike exponential or linear models, the logarithmic form asymptotically saturates---preventing biologically implausible infinite growth in procedural risk.
...
\noindent where $\mathcal{B} = \{50\text{--}54, \; 55\text{--}59, \; \ldots, \; \geq 80\}$ is the set of age bands.

Two mechanisms drive TAM growth:

\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item \textbf{Risk rate growth} ($\hat{R}_i(t)$), which increases but saturates over time due to the logarithmic model.
    \item \textbf{Demographic expansion}, particularly the rapid growth of the $\geq$80 cohort. In South Korea, this cohort is projected to increase from approximately 1.1~million (2012) to 7.3~million (2050); in Singapore, from 33,000 to 567,000 over the same period.
\end{itemize}

The interplay of these two forces produces sustained TAM growth even as individual risk rates plateau.

\subsection{Step 5: TAVI Adoption Modelling via Constrained Sigmoid}
...
\subsubsection{Historical TAVI Share Computation}
...
\noindent where $\tilde{N}_{\mathrm{TAVI}}(t)$ is the redistribution-normalised TAVI volume (Section~2.4) and $\mathrm{TAM}(t)$ is the smoothed TAM derived from the projected risk rates (ensuring consistency with the forward projections).

\subsubsection{Sigmoid Curve Fitting}

TAVI adoption is modelled using a logistic (sigmoid) function with a \textbf{fixed upper asymptote} $L_{\max}$, reflecting the assumption that TAVI will not fully replace SAVR but will converge to a maximum market share:
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item $L_{\max}$ is the maximum TAVI share of TAM (configurable; default 0.50),
    \item $k$ is the logistic growth rate (governing the steepness of adoption),
    \item $t_0$ is the inflection point (year at which adoption reaches $L_{\max}/2$).
\end{itemize}

Parameters $k$ and $t_0$ are estimated by nonlinear least-squares fitting to the historical share data $\{(t, S_{\mathrm{TAVI}}(t))\}$, restricted to years $t \geq 2016$. Data prior to 2016 are excluded from fitting to focus the curve on the period of established clinical adoption, avoiding early-phase noise from programme initiation years.

\paragraph{Bounds.}
Parameter bounds are set as $k \in [0.01, 2.0]$ and $t_0 \in [2000, 2050]$ to ensure physiologically plausible adoption dynamics.

\subsubsection{Rationale for Fixed Upper Asymptote}

The choice to fix $L_{\max}$ rather than the inflection point $t_0$ reflects several considerations:

\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{Clinical reality}---Certain patient subpopulations (e.g., those with bicuspid anatomy, complex coronary disease, or specific annular geometries) will continue to be preferentially treated with SAVR, precluding 100\% TAVI adoption.
    \item \textbf{Model stability}---Fixing the midpoint while allowing $L$ to be free can produce degenerate fits where the curve becomes nearly linear (infinite $L$ with small $k$), particularly when historical data lie entirely on the early, ascending limb of the sigmoid.
    \item \textbf{Interpretability}---The maximum share is a clinically meaningful parameter that can be elicited from clinical expert opinion or benchmarked against international experience in mature TAVI markets.
\end{enumerate}

\subsection{Step 6: TAVI and SAVR Volume Decomposition}
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item If $\hat{S}_{\mathrm{TAVI}}(t)$ grows faster than $\mathrm{TAM}(t)$, SAVR volume declines.
    \item If TAVI share saturates at $L_{\max}$ while demographic forces continue to expand TAM, SAVR volume may stabilise or increase to absorb excess demand.
\end{itemize}

\subsection{Downstream Application: Monte Carlo Simulation}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Prosthetic valve durability}---sampled from age- and valve-type-specific Normal mixture distributions.
    \item \textbf{Patient survival}---sampled from risk-category-specific Normal distributions with optional age-hazard adjustment.
    \item \textbf{Valve failure timing}---computed as the index year plus the drawn durability, discretised to calendar years.
    \item \textbf{ViV candidacy}---determined by requiring valve failure to precede patient death within the forecast horizon.
    \item \textbf{ViV realisation}---subject to a time-varying penetration rate reflecting clinical uptake of the ViV procedure.
\end{itemize}
...
\subsection{Historical Data Profile}

Table~\ref{tab:historical} summarises the historical procedure volumes for both countries at selected years.

\begin{table}[H]
\centering
\caption{Historical TAVI and SAVR Procedure Counts (Selected Years, 2012--2024)}
...
\subsection{Pandemic Redistribution (Step 2)}

The volume-preserved redistribution corrected for COVID-19 disruptions in the 2020--2023 window.

\paragraph{South Korea.}
The scaling factor for combined volumes was $\kappa = 1.071$, indicating that total observed pandemic-era volume exceeded the pre-pandemic linear trend by 7.1\%. This is consistent with pent-up demand and the expansion of TAVI indications during the recovery period.

\paragraph{Singapore.}
The scaling factor was $\kappa = 0.734$, indicating that pandemic-era combined volumes fell 26.6\% below the pre-pandemic trend---a considerably larger disruption than in Korea. This is consistent with Singapore's stringent public health measures, including extended elective surgery deferrals during the circuit-breaker and heightened-alert periods.

\subsection{Risk Rate Projections (Step 3)}

Logarithmic saturation models were fitted independently for each of the seven age bands (50--54 through $\geq$80). In both countries, the $\geq$80 age band exhibited the highest absolute risk rates and the steepest historical growth. By construction, all age-band risk trajectories exhibit diminishing growth rates over the projection horizon, preventing biologically implausible extrapolation beyond the prevalence ceiling.

\subsection{Total Addressable Market (Step 4)}

Table~\ref{tab:tam} presents the projected TAM at selected time horizons, together with the contribution of the $\geq$80 cohort.

\begin{table}[H]
\centering
\caption{Projected Total Addressable Market (TAM) and $\geq$80 Cohort Contribution}
\label{tab:tam}
\smallskip
...
& \multicolumn{3}{c}{\textbf{South Korea}} & \multicolumn{3}{c}{\textbf{Singapore}} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
\textbf{Year} & TAM & $\geq$80 TAM & $\geq$80 Share & TAM & $\geq$80 TAM & $\geq$80 Share \\
\midrule
...
\midrule
\textbf{Growth (2024--2050)} & $\times$2.7 & $\times$4.9 & --- & $\times$3.4 & $\times$4.7 & --- \\
\bottomrule
\end{tabular}
\end{table}

The TAM is projected to grow 2.7-fold in Korea and 3.4-fold in Singapore between 2024 and 2050. In both countries, the $\geq$80 cohort is the overwhelmingly dominant driver:

\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item \textbf{South Korea}: The $\geq$80 population is projected to grow from 2.4~million (2024) to 7.3~million (2050)---a 3.0-fold increase---driving the $\geq$80 TAM contribution from 33.7\% to 60.6\%.
    \item \textbf{Singapore}: The $\geq$80 population is projected to grow from 159{,}000 (2024) to 567{,}000 (2050)---a 3.6-fold increase---driving the $\geq$80 TAM contribution from 42.6\% to 60.1\%.
\end{itemize}

The convergence of both countries to approximately 60\% $\geq$80 TAM share by 2050 reflects a shared demographic trajectory characteristic of developed Asian economies undergoing rapid population ageing.

\subsection{TAVI Adoption Sigmoid Fit (Step 5)}

The constrained sigmoid ($L_{\max} = 0.50$) was fitted to historical TAVI share of TAM for data from 2016 onwards. Table~\ref{tab:sigmoid} presents the fitted parameters.

\begin{table}[H]
\centering
\caption{Sigmoid Curve Parameters for TAVI Share of TAM}
\label{tab:sigmoid}
\smallskip
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Parameter} & \textbf{South Korea} & \textbf{Singapore} & \textbf{Interpretation} \\
\midrule
...
Year to reach 45\% share & $\sim$2023 & $\sim$2040 & Near-saturation timing \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{Korea reaches TAVI--SAVR volume parity by $\sim$2030.} The steep Korean sigmoid results in TAVI and SAVR volumes converging at approximately 2{,}600 each by 2030. Beyond this point, both modalities grow in lockstep at half the TAM.

    \item \textbf{Singapore approaches parity by $\sim$2050.} The more gradual Singaporean adoption curve delays the crossover; by 2050, TAVI (670) approaches but does not exceed SAVR (718), with full parity projected shortly thereafter.

    \item \textbf{Both TAVI and SAVR grow in absolute terms.} This is a critical finding: despite TAVI substituting for SAVR in terms of relative share, the demographic expansion of the elderly population is sufficiently strong that SAVR volumes are projected to increase---not decrease---in both countries. In Korea, SAVR grows from 1{,}992 (2024) to 5{,}095 (2050); in Singapore, from 268 to 718. This has important implications for surgical workforce planning and training, as demand for SAVR capacity will not diminish even as TAVI becomes the dominant modality.

    \item \textbf{Asymptotic equilibrium.} As the TAVI adoption share approaches $L_{\max} = 0.50$, both TAVI and SAVR volumes converge to approximately half of the TAM. Korea reaches this equilibrium by the mid-2030s, while Singapore reaches it by $\sim$2050. In the post-equilibrium phase, total volume growth is driven entirely by demographic forces rather than technology adoption.
\end{enumerate}

\subsection{Sensitivity to the Maximum Share Assumption}

The $L_{\max}$ parameter exerts a first-order effect on the projected volume split. Under alternative assumptions:

\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item \textbf{$L_{\max} = 0.75$}: TAVI would constitute three-quarters of the market at maturity, producing considerably lower SAVR volumes. This scenario reflects potential expansion of TAVI to lower-risk populations currently treated with SAVR.
    \item \textbf{$L_{\max} = 0.30$}: TAVI adoption would plateau at 30\%, preserving SAVR as the dominant approach. This scenario might apply in settings with limited catheterisation laboratory infrastructure or restrictive reimbursement policies.
\end{itemize}
...
First, \textbf{demographic forces will drive sustained growth in total aortic valve procedure demand regardless of TAVI adoption dynamics.} The TAM---representing combined TAVI + SAVR demand---is projected to grow 2.7-fold in Korea and 3.4-fold in Singapore between 2024 and 2050, driven overwhelmingly by the $\geq$80 population expansion. Both countries converge to approximately 60\% of TAM originating from the $\geq$80 cohort by 2050, reflecting the universal demographic transition of developed Asian economies.

Second, \textbf{SAVR volumes are projected to grow in absolute terms despite TAVI substitution.} This counterintuitive finding arises because demographic expansion outpaces the substitution effect under most reasonable adoption ceilings ($L_{\max} \leq 0.50$). Even as TAVI captures half the market, the doubling or tripling of total demand means that SAVR volumes in 2050 are projected to exceed current levels---a finding with important implications for surgical workforce planning.

Third, \textbf{South Korea and Singapore exhibit markedly different adoption dynamics.} Korea's steep sigmoid ($k = 0.502$) with near-saturation by 2024 contrasts sharply with Singapore's gradual curve ($k = 0.096$), which does not approach the adoption ceiling until approximately 2040. These differences likely reflect distinct healthcare system factors including reimbursement timing, regulatory frameworks, centre accreditation policies, and the relative scale of catheterisation laboratory infrastructure.

\subsection{Interpretation in Context}

\subsubsection{TAVI Adoption Patterns: Asia vs.\ the West}
...
\subsubsection{The Demographic Multiplier}
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item In Korea, SAVR volume is projected to grow from approximately 1{,}992 (2024) to 5{,}095 (2050)---a 2.6-fold increase.
    \item In Singapore, SAVR grows from 268 to 718---a 2.7-fold increase.
\end{itemize}

These projections suggest that cardiac surgical capacity will need to \emph{expand}, not contract, to meet future demand---even in a world where TAVI is the dominant modality. This finding is particularly relevant for training programme design, as the pipeline of cardiac surgeons competent in aortic valve surgery will need to grow proportionally.

\subsubsection{Implications for Valve-in-Valve Demand}
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
    \item \textbf{Cumulative TAVI implants (Korea, 2024--2050)}: approximately 94{,}000 valves.
    \item \textbf{Cumulative SAVR implants (Korea, 2024--2050)}: approximately 95{,}000 valves.
    \item \textbf{Combined}: nearly 190{,}000 bioprostheses requiring surveillance for SVD.
\end{itemize}
...
\subsubsection{Pandemic Redistribution}
...
\subsubsection{Logarithmic Risk Rate Saturation}
...
\subsubsection{Fixed vs.\ Free Maximum Share}
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{Fixed maximum share assumption.} The use of a fixed $L_{\max} = 0.50$ assumes that TAVI will capture at most half of the aortic valve market. In reality, the ceiling may evolve as clinical evidence accumulates, valve technology improves, and indications expand. If low-risk trial data continue to favour TAVI beyond 10-year follow-up, the true ceiling could exceed 0.50, and our SAVR projections would be overestimates. Conversely, if long-term structural valve deterioration proves more prevalent than anticipated, the ceiling could be lower.

    \item \textbf{Deterministic TAM projection.} The TAM is computed as a deterministic product of projected populations and risk rates, without uncertainty quantification. In practice, population projections carry uncertainty (driven by fertility, mortality, and migration assumptions), and risk rate convergence to prevalence ceilings may be faster or slower than the logarithmic model predicts. A future extension could incorporate prediction intervals by propagating demographic uncertainty through the model.

    \item \textbf{Country-level granularity.} The model operates at the national level and does not account for within-country geographic heterogeneity in access to TAVI. In large countries like South Korea, regional variation in centre availability, referral patterns, and patient willingness to travel may produce local demand patterns that differ substantially from the national aggregate.

    \item \textbf{Static risk mix.} The model assumes that the age-specific risk rate captures both incidence and treatment propensity as a single quantity. It does not separately model changes in treatment thresholds, clinical guidelines, or patient selection criteria that could shift the risk profile of treated patients over time.

    \item \textbf{Limited validation data.} With only 9--12 years of historical data, out-of-sample validation is constrained. Cross-validation against withheld years (e.g., fitting to 2012--2020 and validating against 2021--2024) could provide some assessment of predictive performance, but the short forecast horizon reduces the power of such tests.

    \item \textbf{Absence of economic and policy dynamics.} The model does not explicitly account for policy changes such as reimbursement expansions, new device approvals, or changes in centre accreditation criteria---all of which could accelerate or decelerate adoption. These are implicitly captured to the extent that they are reflected in the historical data used for sigmoid fitting.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{Uncertainty quantification.} Incorporating bootstrap or Bayesian approaches to estimate confidence intervals on the sigmoid parameters, propagate demographic uncertainty, and produce probabilistic volume forecasts.

    \item \textbf{Additional countries.} The framework is explicitly designed to be country-agnostic. Application to Japan, Taiwan, and other Asian healthcare systems would enable cross-country comparison of adoption dynamics and identification of system-level factors that accelerate or delay TAVI diffusion.

    \item \textbf{Dynamic $L_{\max}$.} Replacing the fixed maximum share with a time-varying ceiling that responds to guideline changes, trial results, or policy interventions. This could be implemented as a piecewise function or as a second-level sigmoid.

    \item \textbf{Integration with ViV forecasting.} The volume projections from this framework are already used as inputs to a patient-level Monte Carlo simulation of ViV demand. Tighter integration---where ViV demand feeds back to influence repeat procedure capacity---would create a fully closed-loop forecasting system.

    \item \textbf{Sub-national modelling.} Extension to province- or city-level forecasting, incorporating geographic access models and centre-specific capacity data, would provide more actionable planning inputs for hospital administrators and health authorities.
\end{enumerate}
...
We have presented a six-step, demographically anchored forecasting framework for TAVI and SAVR procedure volumes that explicitly separates technology adoption dynamics from the demographic forces driving total demand. Applied to South Korea and Singapore, the framework reveals three key insights: (1)~total aortic valve procedure demand will grow 2.7--3.4$\times$ by 2050, driven primarily by the expansion of the $\geq$80 population; (2)~SAVR volumes will grow in absolute terms despite declining market share, challenging assumptions that TAVI growth will reduce surgical demand; and (3)~the two countries exhibit markedly different adoption trajectories, reflecting distinct healthcare system characteristics despite shared demographic trends.
