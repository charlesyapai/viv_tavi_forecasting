...
def main():
    ...
    parser.add_argument("--sigmoid-mode", choices=["free", "fixed_cap", "fixed_midpoint"], default="free", 
                        help="Sigmoid fitting mode: 'free' (fit all), 'fixed_cap' (fix L), 'fixed_midpoint' (fix x0)")
    ...
    # Filter Data for Fitting (Start Year onwards)
    fit_x = []
    fit_y = []
    for y, s in zip(valid_hist_years, tavi_shares):
        if y >= args.start_year:
            fit_x.append(y)
            fit_y.append(s)
            
    if len(fit_x) < 3:
        log.warning(f"Not enough data points >= {args.start_year} for fit. Using all combined data.")
        fit_x = valid_hist_years
        fit_y = tavi_shares

    # Define Sigmoid Variants
    def sigmoid_free(x, L, k, x0):
        return L / (1 + np.exp(-k * (x - x0)))
        
    def sigmoid_fixed_L(x, k, x0):
        return tavi_max_share / (1 + np.exp(-k * (x - x0)))

    # Midpoint fix mode (user requested fixing midpoint to 2013 or similar)
    if args.sigmoid_mode == "fixed_midpoint":
        fixed_x0 = args.fixed_midpoint_val
        log.info(f"Fitting with FIXED MIDPOINT (x0={fixed_x0})")
        
        def sigmoid_fixed_x0(x, L, k):
            return L / (1 + np.exp(-k * (x - fixed_x0)))
            
        # Guess: L=0.8, k=0.5
        p0 = [0.8, 0.5]
        bounds = ([0.0, 0.01], [1.0, 5.0])
        try:
            popt, _ = curve_fit(sigmoid_fixed_x0, fit_x, fit_y, p0=p0, bounds=bounds, maxfev=10000)
            L_fit, k_fit = popt
            x0_fit = fixed_x0
            log.info(f"Sigmoid Fit (x0={x0_fit}): L={L_fit:.2f}, k={k_fit:.3f}")
        except Exception as e:
            log.error(f"Fixed Midpoint Fit failed: {e}")
            L_fit, k_fit, x0_fit = 0.5, 0.2, fixed_x0

    elif args.sigmoid_mode == "fixed_cap":
        log.info(f"Fitting with FIXED CAP (L={tavi_max_share:.2%})")
        # Guess: k=0.2, x0=2022
        p0 = [0.2, 2022]
        bounds = ([0.01, 2000], [2.0, 2050])
        try:
            popt, _ = curve_fit(sigmoid_fixed_L, fit_x, fit_y, p0=p0, bounds=bounds, maxfev=10000)
            k_fit, x0_fit = popt
            L_fit = tavi_max_share
            log.info(f"Sigmoid Fit (L={L_fit}): k={k_fit:.3f}, x0={x0_fit:.1f}")
        except Exception as e:
            log.error(f"Fixed Cap Fit failed: {e}")
            k_fit, x0_fit = 0.1, 2025
            L_fit = tavi_max_share

    else: # Default: Free Fit
        log.info("Fitting with FREE PARAMETERS (L, k, x0)")
        # Guess: L=0.8, k=0.5, x0=2024
        p0 = [0.8, 0.5, 2024]
        bounds = ([0.0, 0.01, 2010], [1.0, 5.0, 2050])
        try:
            popt, _ = curve_fit(sigmoid_free, fit_x, fit_y, p0=p0, bounds=bounds, maxfev=10000)
            L_fit, k_fit, x0_fit = popt
            log.info(f"Sigmoid Fit (Free): L={L_fit:.2f}, k={k_fit:.3f}, x0={x0_fit:.1f}")
        except Exception as e:
            log.error(f"Free Fit failed: {e}")
            L_fit, k_fit, x0_fit = 0.5, 0.2, 2025
            
    # 3. Project Future Shares & Volumes
    # ---------------------------------------------------------
    pred_shares = sigmoid_free(proj_years_full, L_fit, k_fit, x0_fit)
    ...
    ax_v3.plot(excl_x, excl_y, 'o', color='gray', alpha=0.5, label=f"Historical < {args.start_year} (Excluded)")
    ax_v3.plot(incl_x, incl_y, 'bo', label=f"Historical >= {args.start_year} (Fitted)")
    
    # Plot Curve
    ax_v3.plot(proj_years_full, pred_shares, 'b-', linewidth=2, label=f"Sigmoid (L={L_fit:.2f}, x0={x0_fit:.1f})")
    
    # Formatting
    ax_v3.set_ylim(0, 1.05) 
    ax_v3.axhline(L_fit, color='r', linestyle=':', label=f"Fit Saturation ({L_fit:.1%})")
