...
>>>>>>> Stashed changes
    ...
    # Singapore CSVs are already in long format based on model_v11_singapore.yaml?
    # model_v11_singapore.yaml says: data/singapore_registry_tavi.csv
    # Let's check if they require 'process_wide' or are already long.
    # The 'load_raw_wide_data' function assumes TAVI/SAVR RAW structure (wide).
    # If Singapore data is already long (year, sex, age_band, count), we should just read it.
    
    # I will verify the format of singapore_registry_tavi.csv first using a separate specific read if needed, 
    # but based on 'model_v11.py' dealing with 'procedure_counts["tavi"]' as 'pd.read_csv(...)',
    # and model_v11_singapore.yaml pointing to them, they are likely already in the format 
    # expected by model_v11 (which is long).
    
    # So I should REPLACE load_raw_wide_data content to just read the long CSVs and aggregate/format if needed.
    # model_v11 uses: sex,age_band,year,count.
    # final_adoption_model needs: year, age_band, count (summed over sex).
    
    tavi_df = pd.read_csv(tavi_raw)
    savr_df = pd.read_csv(savr_raw)
    
    # Standardize columns to lower case
    tavi_df.columns = [c.lower() for c in tavi_df.columns]
    savr_df.columns = [c.lower() for c in savr_df.columns]
    
    # Ensure columns exist
    # Expected: year, age_band, count (maybe sex)
    
    def prep_long(df):
        # sum over sex if exists
        if 'sex' in df.columns:
            df = df.groupby(['year', 'age_band'])['count'].sum().reset_index()
        return df[['year', 'age_band', 'count']]
        
    tavi_df = prep_long(tavi_df)
    savr_df = prep_long(savr_df)
...
def load_data():
    ...
    hosp_path = Path("/Users/charles/Desktop/viv_tavi_forecasting/data/raw/TAVI_adoption_hospital_numbers_singapore") # Assuming this exists or using a dummy if not critical
    ...
    un_m_path = Path("/Users/charles/Desktop/viv_tavi_forecasting/simulation_run_v1/data/un_population_data/Population Projections - Male, Singapore.csv")
    un_f_path = Path("/Users/charles/Desktop/viv_tavi_forecasting/simulation_run_v1/data/un_population_data/Population Projections - Female, Singapore.csv")
    ...
    def process_un(p):
        df = pd.read_csv(p)
        df = df[df["Region, subregion, country or area *"] == "Singapore"]
...
>>>>>>> Stashed changes
    ...
    ax1.set_title("Input 1: Index Procedure Volumes (Singapore)", fontsize=12)
