...
/Users/charles/miniconda3/bin/python /Users/charles/Desktop/viv_tavi_forecasting/simulation_run_v1/models/model_v5.py --config configs/model_v5_configs.yaml --log-level DEBUG
...
def extrapolate_volumes(csv_path: Path, end_year: int,
                        ...
                        out_dir: Path,
                        method: str = "linear",
                        const_from_year: int | None = None,
                        const_value: int | None = None) -> pd.DataFrame:
    """
    • Reads registry CSV (year, age_band, sex, count)
    • Appends synthetic rows up to end_year
       - method='linear' → linear trend on *totals* over last `window` years
       - method='constant' → from `const_from_year`, total fixed at `const_value`
    • Returns full DataFrame (observed + synthetic) with 'src' column
    • Saves CSV + a QC PNG plot
    """
    df = pd.read_csv(csv_path).assign(src="observed")
    last_year = int(df.year.max())

    # --- build per-year totals for projection ---
    observed_totals = df.groupby("year")["count"].sum().sort_index()

    if method == "constant":
        if const_from_year is None or const_value is None:
            raise ValueError("constant method requires const_from_year and const_value")
    ...
    # --- placeholder future rows (age pattern fixed later) ---
    synth = pd.DataFrame({
        "year": pred_years.repeat(1),
        "age_band": "75-79",   # will be redistributed by age projection
        "sex": "U",
        "count": preds,
        "src": "extrap"
    })
    ...
    # QC plot
    plt.figure()
    plt.plot(observed_totals.index, observed_totals.values, label="observed")
    if len(pred_years):
        plt.plot(pred_years, preds, "--", label=("constant" if method=="constant" else "linear fit"))
...
def _age_band_shares_for_year(pop_df: pd.DataFrame, year: int,
    ...
    """Population shares per registry age_band for a given year."""
...
def redistribute_future_by_population(df: pd.DataFrame,
    ...
    """
    For rows with src='extrap' (one per future year with placeholder age_band),
    expand into multiple rows allocated across existing registry age_bands
    using population shares for that year.
    """
...
class ViVSimulator:
    ...
    def __init__(self, cfg: Config, out_dir: Path):
        ...
        # TAVI: keep linear
        self.tavi_df = extrapolate_volumes(
            cfg.procedure_counts["tavi"],
            self.end_year, win, self.open_width, out_dir,
            method="linear"
        )

        # SAVR: constant 2,000 from 2025 (configurable)
        savr_cfg = cfg.volume_extrapolation.get("savr", {})
        self.savr_df = extrapolate_volumes(
            cfg.procedure_counts["savr"],
            self.end_year, win, self.open_width, out_dir,
            method=savr_cfg.get("method", "constant"),
            const_from_year=int(savr_cfg.get("from_year", 2025)),
            const_value=int(savr_cfg.get("value", 2000)),
        )
        log.info("TAVI rows %d, SAVR rows %d (observed+synthetic)",
                 len(self.tavi_df), len(self.savr_df))


        # --- population-based age allocation for FUTURE years ---
        pop_cfg = self.cfg.dict().get("population_projection")
        if pop_cfg and Path(pop_cfg["path"]).exists():
            pop_df = pd.read_csv(pop_cfg["path"])
            ycol, acol, pcol = pop_cfg["year_col"], pop_cfg["age_col"], pop_cfg["pop_col"]
            self.tavi_df = redistribute_future_by_population(self.tavi_df, pop_df,
                                                            self.open_width, ycol, acol, pcol)
            self.savr_df = redistribute_future_by_population(self.savr_df, pop_df,
                                                            self.open_width, ycol, acol, pcol)
        else:
            log.warning("Population projection not found/configured; future age mix will be flat placeholder.")
...
def _age_labels(edges):
    """Return list like ['<60', '60-64', ..., '≥80'] matching `edges` + open bin."""
    labs = [f"<{edges[0]}"]
    for lo, hi in zip(edges[:-1], edges[1:]):
        labs.append(f"{lo}-{hi-1}")
    labs.append(f"≥{edges[-1]}")  # NEW: open-ended
    return labs
