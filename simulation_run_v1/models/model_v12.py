...
class ProcProjection(BaseModel):
    method: str = "linear"                          # linear | constant | population_rate | external
    window: int = 5
    ...
    def _method_ok(cls, v):
        if v not in ("linear", "constant", "population_rate", "external"):
            raise ValueError("index_projection.method must be linear|constant|population_rate|external")
        return v
...
class PrecomputeConfig(BaseModel):
    ...
    # NEW: Select which country data to use
    active_country: str = "korea" # "korea" or "singapore"
    
    # NEW: Paths to the UN data files
    un_population_paths: Dict[str, str] = {
    ...
    # Multiplier: UN data is usually in Thousands. If raw data is "1 051", that means 1,051,000.
    # Adjust this based on your specific CSV. 
    # If the CSV says "803" and it means 803 people, use 1.0. 
    # If it means 803,000, use 1000.0.
    units_multiplier: float = 1000.0 

    risk_years: List[int] = [2023, 2024]
    risk_rule: str = "avg_2023_2024"
    project_years: List[int] = [2025, 2050]

    # Legacy settings used for risk score alignment, kept for compatibility
...
# =============================================================================
# Demography pre-compute (UN Data Format)
# =============================================================================

def _parse_un_value(val: Any) -> float:
    """Handle UN formatting e.g. '1 051' -> 1051.0"""
    if pd.isna(val):
        return 0.0
    s = str(val).strip()
...
def _expand_un_bin(
    year: int, sex: str, band_label: str, count: float, out_rows: list
) -> None:
    """
    Expand a UN bin (e.g. '10-14' or '100+') into single year ages.
    Distributes count uniformly (or via largest remainder) across the ages.
    """
    # Parse band "10-14" -> 10..14; "100+" -> 100..104 (arbitrary cap width)
    band_label = band_label.strip()
    if "+" in band_label:
        # e.g. 100+
        start_age = int(re.match(r"(\d+)", band_label).group(1))
        # We assume a cap width of 5 for the final bin to keep it simple, 
        # or we can go higher. Let's use 5 to match the other bands.
        end_age = start_age + 5 
    else:
        # e.g. 0-4
        parts = band_label.split("-")
        if len(parts) != 2:
            return # Skip weird columns
        start_age = int(parts[0])
        end_age = int(parts[1]) + 1 # Exclusive

    n_years = end_age - start_age
    if n_years <= 0:
        return

    # Distribute integer population
    total_pop = int(round(count))
    shares = np.ones(n_years, dtype=float)
    alloc = _largest_remainder_split(total_pop, shares)

    for i, n in enumerate(alloc):
        age = start_age + i
        if n > 0:
            out_rows.append([year, age, sex, n])

def build_age_and_sex_population_un(
    male_path: Path,
    female_path: Path,
    out_dir: Path,
    ...
    """
    Parses UN Population Projection CSVs.
    Format expected: Region, Gender, Year, 0-4, 5-9, ... 100+
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    y_min, y_max = project_span

    # Container for single-year data: [year, age, sex, population]
    single_year_rows = []

    for sex_label, path in [("Men", male_path), ("Women", female_path)]:
        if not Path(path).exists():
            raise FileNotFoundError(f"UN population file not found: {path}")
        
        df = pd.read_csv(path)
        
        # Identify age columns (usually 0-4, 5-9, ... 100+)
        # We skip 'Region', 'Gender', 'Year'
        meta_cols = ["Region", "subregion", "country", "area", "Gender", "Year", 
                     "Index", "Variant", "Notes", "Location", "ISO", "SDMX", "Type", "Parent"]
        age_cols = [c for c in df.columns if not any(m.lower() in c.lower() for m in meta_cols)]
        ...
        for _, row in df.iterrows():
            ...
            for col in age_cols:
                val_raw = row[col]
                val_float = _parse_un_value(val_raw) * units_multiplier
                _expand_un_bin(year, sex_label, col, val_float, single_year_rows)

    # 1. Create the Master Single-Year DataFrame
    by_age_sex = pd.DataFrame(single_year_rows, columns=["year", "age", "sex", "population"])
    
    # 2. Create the "All Sex" DataFrame (for Index Projection)
    by_age_all = (by_age_sex.groupby(["year", "age"])["population"].sum()
                  .reset_index())

    # 3. Create the "Band" DataFrame (Year x Sex x Band)
    # This is needed for the legacy risk_score calculator (baseline_risk_scores_by_sex.csv)
    # We will aggregate single years back into standard 5-year bands 
    # to match the registry data format (usually 50-54, 55-59...).
    
    # Define standard bands
    def get_band(a: int) -> str:
        if a >= 85: return "≥85"
        base = (a // 5) * 5
        return f"{base}-{base+4}"

    by_age_sex["temp_band"] = by_age_sex["age"].apply(get_band)
    by_band_sex = (by_age_sex.groupby(["year", "sex", "temp_band"])["population"]
                   .sum().reset_index().rename(columns={"temp_band": "age_band"}))
    ...
    by_age_sex.drop(columns=["temp_band"], errors="ignore").to_csv(p_age_sex, index=False)
    by_age_all.to_csv(p_age_all, index=False)

    log.info(f"Built population files from UN data (Years {y_min}-{y_max})")
...
def _plot_adoption_trends(tavi_csv: Path, savr_csv: Path, out_dir: Path) -> None:
    """
    Generate historical and projected adoption plots for TAVI vs SAVR.
    """
    if not tavi_csv.exists() or not savr_csv.exists():
        log.warning("Missing TAVI/SAVR projection files; skipping adoption plots.")
        return

    out_dir.mkdir(parents=True, exist_ok=True)

    # Load and combine
    tavi = pd.read_csv(tavi_csv)
    savr = pd.read_csv(savr_csv)
    tavi["procedure"] = "TAVI"
    savr["procedure"] = "SAVR"
    
    df = pd.concat([tavi, savr], ignore_index=True)
    
    # Map 'pop_rate' (or other future sources) to 'projected'
    # 'observed' stays 'observed'
    def map_src(s):
        return "observed" if s == "observed" else "projected"
    
    df["plot_src"] = df["src"].apply(map_src)

    # 1. Historical (Side-by-Side Bar)
    obs = df[df["plot_src"] == "observed"].copy()
    if not obs.empty:
        agg_hist = obs.groupby(["year", "procedure"])["count"].sum().reset_index()
        
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.barplot(data=agg_hist, x="year", y="count", hue="procedure", 
                    palette=["#1f77b4", "#ff7f0e"], ax=ax)
        ax.set_title("Historical Adoption: TAVI vs SAVR")
        ax.set_xlabel("Year")
        ax.set_ylabel("Total Procedures")
        ax.grid(axis="y", alpha=0.3)
        _savefig_current(out_dir / "historical_adoption.png")

    # 2. Projected (Line Chart)
    agg_proj = df.groupby(["year", "procedure", "plot_src"])["count"].sum().reset_index()
    
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.lineplot(data=agg_proj, x="year", y="count", hue="procedure", style="plot_src",
                 markers=True, dashes={"observed": "", "projected": (2, 2)},
                 palette=["#1f77b4", "#ff7f0e"], ax=ax)
    
    # Vertical line at end of observed
    last_obs = agg_proj[agg_proj["plot_src"] == "observed"]["year"].max()
    if pd.notna(last_obs):
        ax.axvline(x=last_obs + 0.5, color="gray", linestyle="--", alpha=0.5, label="Projection Start")
        
    ax.set_title("Projected Adoption Trends: TAVI vs SAVR")
    ax.set_xlabel("Year")
    ax.set_ylabel("Total Procedures")
    ax.grid(True, alpha=0.3)
    _savefig_current(out_dir / "projected_adoption.png")
...
def _plot_demography(by_band_sex_csv: Path,
    ...
    # --- Combined (Men + Women) Age Projection Lines ---
    combined_band = band_df.groupby(["year", "age_band"])["population"].sum().reset_index()
    if not combined_band.empty:
        plt.figure(figsize=(10, 6), dpi=140)
        for b in bands_order:
            grp = combined_band[combined_band["age_band"] == b].sort_values("year")
            if grp.empty:
                continue
            y_vals = grp["population"].astype(float) / float(units_scale)
            plt.plot(grp["year"], y_vals, marker="o", label=b)
        ...
        ax.set_title("Projected population by age band (Combined)")
        ax.set_xlabel("Year")
        ax.set_ylabel(f"Population (×{int(units_scale):,} people)")
        ax.legend(ncol=2, fontsize=9)
        plt.tight_layout()
        _savefig_current(out_dir / "age_projection_lines_Combined.png")
...
def _plot_population_summary(age_all_csv: Path, out_dir: Path, units_scale: float, cfg: Optional[Config] = None) -> None:
    """
    Generates a summary plot showing:
      1) Total Population
      2) Population >= 50
      3) Average Age (Total)
      4) Average Age (>= 50)
    ...
    # Pre-calculate product for weighted average
    df["age_pop_prod"] = df["age"] * df["population"]
    ...
    for y in years:
        ...
        tot_pop = sub["population"].sum()
        tot_prod = sub["age_pop_prod"].sum()
        avg_age_tot = (tot_prod / tot_pop) if tot_pop > 0 else 0.0
        
        # --- >= 50 Population Metrics ---
        sub50 = sub[sub["age"] >= 50]
        pop50 = sub50["population"].sum()
        prod50 = sub50["age_pop_prod"].sum()
        avg_age_50 = (prod50 / pop50) if pop50 > 0 else 0.0
        
        stats.append({
            "year": y,
            "pop_total": tot_pop,
            "pop_50plus": pop50,
            "avg_age_total": avg_age_tot,
            "avg_age_50plus": avg_age_50
        })
        
    res = pd.DataFrame(stats)
    # Save the raw data for validation
    res.to_csv(out_dir / "population_summary_metrics.csv", index=False)

    # --- Plotting (Separate Figures) ---

    # 1. Population Trends
    fig1, ax1 = plt.subplots(figsize=(10, 6), dpi=140)
    ax1.plot(res["year"], res["pop_total"] / units_scale, label="Total Population", 
             color="black", linewidth=2.5)
    ax1.plot(res["year"], res["pop_50plus"] / units_scale, label="Population (Age ≥50)", 
             color="#0055aa", linewidth=2.5) # Strong Blue
    
    ax1.set_ylabel(f"Population (×{int(units_scale):,} people)")
    ax1.set_xlabel("Year")
    ax1.set_title("Population Trends")
    ax1.legend(loc="upper left")
    ax1.grid(True, alpha=0.3)
    _apply_xrange_and_focus(ax1, cfg)
    
    fig1.tight_layout()
    _savefig_current(out_dir / "demography_summary_trends_population.png")

    # 2. Average Age Trends
    fig2, ax2 = plt.subplots(figsize=(10, 6), dpi=140)
    ax2.plot(res["year"], res["avg_age_total"], label="Avg Age (Total Pop)", 
             color="gray", linewidth=2.0, linestyle="--")
    ax2.plot(res["year"], res["avg_age_50plus"], label="Avg Age (Pop ≥50)", 
             color="#aa0000", linewidth=2.5) # Dark Red
    
    ax2.set_ylabel("Average Age (Years)")
    ax2.set_xlabel("Year")
    ax2.set_title("Average Age Trends")
    ax2.legend(loc="upper left")
    ax2.grid(True, alpha=0.3)
    _apply_xrange_and_focus(ax2, cfg)
    
    fig2.tight_layout()
    _savefig_current(out_dir / "demography_summary_trends_average_age.png")

    # Keep original combined plot for backward compatibility if desired, 
    # or just rely on the separate ones. The user asked for "also be created seperately",
    # so we'll keep the combined one too as "demography_summary_trends.png" (legacy).
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 10), dpi=140, sharex=True)
    
    # Subplot 1: Population Counts
    ax1.plot(res["year"], res["pop_total"] / units_scale, label="Total Population", 
             color="black", linewidth=2.5)
    ax1.plot(res["year"], res["pop_50plus"] / units_scale, label="Population (Age ≥50)", 
             color="#0055aa", linewidth=2.5)
    ...
    # Subplot 2: Average Ages
    ax2.plot(res["year"], res["avg_age_total"], label="Avg Age (Total Pop)", 
             color="gray", linewidth=2.0, linestyle="--")
    ax2.plot(res["year"], res["avg_age_50plus"], label="Avg Age (Pop ≥50)", 
             color="#aa0000", linewidth=2.5)
    ...
    plt.tight_layout()
    _savefig_current(out_dir / "demography_summary_trends.png")
...
def _project_index_from_external(
    obs_df: pd.DataFrame,
    end_year: int,
    external_file: str,
    external_col: str
    ...
    """
    Project index volumes using an external CSV file.
    Expects external file to have 'year' and the specified column.
    """
    df = obs_df.copy()
    df["src"] = "observed"
    
    if not Path(external_file).exists():
        raise FileNotFoundError(f"External projection file not found: {external_file}")

    ext_df = pd.read_csv(external_file)
    if external_col not in ext_df.columns:
        raise ValueError(f"Column '{external_col}' not found in {external_file}. Available: {list(ext_df.columns)}")
        
    if "year" not in ext_df.columns:
         raise ValueError(f"Column 'year' not found in {external_file}")
         
    # Filter for future years
    last_obs_year = int(df["year"].max())
    future_mask = (ext_df["year"] > last_obs_year) & (ext_df["year"] <= end_year)
    future_data = ext_df.loc[future_mask].copy()
    
    rows = []
    for _, row in future_data.iterrows():
        # Handle NaN or weird values
        val = row[external_col]
        if pd.isna(val):
            val = 0
        ...
        rows.append({
            ...
            "sex": "U",
            "count": int(val),
            "src": "external" # Distinguish from extrap
        })
    ...
    # Treat 'external' same as 'extrap' for redistribution purposes
    synth["src"] = "extrap" 
...
class ViVSimulator:
    def __init__(self, cfg: Config, dirs: Dirs):
        ...
        elif ip.tavi and ip.tavi.method == "external":
             self.tavi_df = _project_index_from_external(
                tavi_obs, self.end_year, 
                ip.tavi.external_file, ip.tavi.external_col
            )
        ...
        elif ip.savr and ip.savr.method == "external":
             self.savr_df = _project_index_from_external(
                savr_obs, self.end_year, 
                ip.savr.external_file, ip.savr.external_col
            )
             if pop_df is not None:
...
def _plot_demographic_contribution(index_csv: Path, out_path: Path, title: str, cfg: Config) -> None:
    """
    Plot the contribution of each age band to the total volume over time (Stacked Area Chart).
    This visualizes how shifting demographics impact total volume.
    """
    df = pd.read_csv(index_csv)
    if df.empty:
        return

    # Group by year and age_band
    # df columns: year, age_band, sex, count, src
    
    # We aggregate over sex
    grouped = df.groupby(["year", "age_band"])["count"].sum().unstack(fill_value=0)
    
    # Sort columns (age bands) based on logic
    try:
        sorted_cols = sorted(grouped.columns, key=lambda c: parse_age_band(c, cfg.open_age_bin_width)[0])
    except:
        sorted_cols = sorted(grouped.columns)
        
    grouped = grouped[sorted_cols]
    
    # Filter for plot range
    min_year = cfg.figure_ranges.get("x_axis", [2022, 2050])[0] if cfg.figure_ranges else 2022
    max_year = cfg.figure_ranges.get("x_axis", [2022, 2050])[1] if cfg.figure_ranges else 2050
    ...
    # Stacked Area Plot
    ax.stackplot(grouped.index, grouped.T.values, labels=grouped.columns, alpha=0.8)
    
    ax.set_title(title)
    ax.set_xlabel("Year")
    ax.set_ylabel("Procedures / yr")
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1), title="Age Band")
...
def build_index_projection(cfg: Config, out_dir: Path, pop_proj_csv: Path) -> Tuple[Path, Path]:
    """
    Build index projections for TAVI and SAVR, saving them to out_dir.
    Returns (tavi_csv_path, savr_csv_path).
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    pop_df = None
    pop_cols = ("year", "age", "population")
    if pop_proj_csv and pop_proj_csv.exists():
        pop_df = pd.read_csv(pop_proj_csv)
        # Assuming standard columns for the internal file
        pop_cols = ("year", "age", "population")
    
    ip = cfg.index_projection
    end_year = cfg.years["end"]
    open_width = cfg.open_age_bin_width
    
    # TAVI
    if ip and ip.tavi and ip.tavi.method == "population_rate" and pop_df is not None:
        tavi_df = _project_index_by_pop_rate(
            tavi_obs, end_year, pop_df, open_width, *pop_cols,
            ip.tavi.pop_rate or PopRateSettings()
        )
    else:
        # Fallback / default
        method = ip.tavi.method if (ip and ip.tavi) else "linear"
        window = ip.tavi.window if (ip and ip.tavi) else 5
        tavi_df = _linear_or_constant(tavi_obs, end_year, window, method=method)
        if pop_df is not None:
             tavi_df = redistribute_future_by_population(tavi_df, pop_df, open_width, *pop_cols)

    # SAVR
    if ip and ip.savr and ip.savr.method == "population_rate" and pop_df is not None:
        savr_df = _project_index_by_pop_rate(
            savr_obs, end_year, pop_df, open_width, *pop_cols,
            ip.savr.pop_rate or PopRateSettings()
        )
    else:
        method = ip.savr.method if (ip and ip.savr) else "constant"
        window = ip.savr.window if (ip and ip.savr) else 5
        const_from = ip.savr.constant.from_year if (ip and ip.savr and ip.savr.constant) else 2025
        const_val  = ip.savr.constant.value if (ip and ip.savr and ip.savr.constant) else 2000
        
        savr_df = _linear_or_constant(
            savr_obs, end_year, window,
            method=method, const_from_year=const_from, const_value=const_val
        )
        if pop_df is not None:
            savr_df = redistribute_future_by_population(savr_df, pop_df, open_width, *pop_cols)

    p_tavi = out_dir / "tavi_with_projection.csv"
    p_savr = out_dir / "savr_with_projection.csv"
    
    tavi_df.to_csv(p_tavi, index=False)
    savr_df.to_csv(p_savr, index=False)
    
    log.info("Built index projection: TAVI rows %d, SAVR rows %d", len(tavi_df), len(savr_df))
    return p_tavi, p_savr
...
def main():
    ...
    if cfg.precompute and cfg.precompute.enabled:
        ...
        # Determine paths based on active country       
        c = pre.active_country.lower()
        if c == "singapore":
            m_path = pre.un_population_paths["singapore_male"]
            f_path = pre.un_population_paths["singapore_female"]
        else:
            m_path = pre.un_population_paths["korea_male"]
            f_path = pre.un_population_paths["korea_female"]

        # Call the new UN parser
        p_band_sex, p_age_sex, p_age_all = build_age_and_sex_population_un(
            male_path=Path(m_path),
            female_path=Path(f_path),
            out_dir=derived_dir,
            units_multiplier=float(pre.units_multiplier),
        ...
        # 2. Index Projection (TAVI / SAVR)
        #    This creates tavi_with_projection.csv and savr_with_projection.csv
        p_tavi_idx, p_savr_idx = build_index_projection(
            cfg=cfg,
            out_dir=dirs.tables_index,
            pop_proj_csv=p_age_all
        )

        # --- NEW: Plot Adoption Trends immediately ---
        _plot_adoption_trends(p_tavi_idx, p_savr_idx, dirs.fig_index)
        
        # --- NEW: Plot Demographic Contribution ---
        _plot_demographic_contribution(p_tavi_idx, dirs.fig_index / "tavi_demographic_contribution.png", "TAVI Volume by Age Band Contribution", cfg)
        _plot_demographic_contribution(p_savr_idx, dirs.fig_index / "savr_demographic_contribution.png", "SAVR Volume by Age Band Contribution", cfg)
        ...
        try:
            ...
            # --- NEW: Call the summary plot ---
            _plot_population_summary(
                p_age_all,       # Uses the single-year all-sex file
                dirs.fig_demog,  # Outputs to figures/demography
                units_scale=units,
                cfg=cfg
            )
            # ----------------------------------
